...
public class UserConversor {
	...
	public final static UserDto toUserDto(User user) {
		return new UserDto(user.getId(), user.getUserName(), user.getHotelName(), user.getAddress(), user.getEmail(), 
			user.getPhone(), user.getRole().toString());
	}
	
	public final static User toUser(UserDto userDto) {
		
		return new User(userDto.getUserName(), userDto.getPassword(), userDto.getHotelName(), userDto.getAddress(),
